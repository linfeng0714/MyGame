...
namespace SqlCipher
{
    ...
    public static partial class SQLite3
    {
        ...
        public static extern Result Open_V2(byte[] filename, out Sqlite3DatabaseHandle db, int flags,
            Sqlite3DatabaseHandle zvfs);

        [DllImport(DLL_NAME, EntryPoint = "sqlite3_open", CallingConvention = CallingConvention.Cdecl)]
        internal static extern Result Open(byte[] filename, out Sqlite3DatabaseHandle db);

        [DllImport(DLL_NAME, EntryPoint = "sqlite3_key", CallingConvention = CallingConvention.Cdecl)]
        public static extern Result Key(Sqlite3DatabaseHandle db, byte[] key, int keylen);

        [DllImport(DLL_NAME, EntryPoint = "sqlite3_rekey", CallingConvention = CallingConvention.Cdecl)]
        public static extern Result ReKey(Sqlite3DatabaseHandle db, byte[] key, int keylen);
        ...
        [DllImport(DLL_NAME, EntryPoint = "sqlite3_prepare_v2", CallingConvention = CallingConvention.Cdecl)]
        public static extern Result Prepare2_UTF8(Sqlite3DatabaseHandle db, byte[] sql,
    int numBytes, out Sqlite3DatabaseHandle stmt, Sqlite3DatabaseHandle pzTail);
        ...
        public static extern Sqlite3DatabaseHandle Errmsg16(Sqlite3DatabaseHandle db);

        [DllImport(DLL_NAME, EntryPoint = "sqlite3_errmsg", CallingConvention = CallingConvention.Cdecl)]
        ...
        public static Sqlite3DatabaseHandle Prepare2(Sqlite3DatabaseHandle db, string query, bool utf8)
        {
            Sqlite3DatabaseHandle stmt;
            Result r;
            if (utf8)
            {
                byte[] arr = Encoding.UTF8.GetBytes(query);
                r = Prepare2_UTF8(db, arr, arr.Length, out stmt, Sqlite3DatabaseHandle.Zero);
            }
            else
            {
                r = Prepare2(db, query, query.Length, out stmt, Sqlite3DatabaseHandle.Zero);
            }
