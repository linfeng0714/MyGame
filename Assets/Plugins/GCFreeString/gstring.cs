...
namespace System
{
    public class gstring
    {
        ...
        static Dictionary<string, string> g_intern_table;
        ...
        static gstring Empty = new gstring(0);
        ...
        {
            string interned;
            if (g_intern_table.TryGetValue(value, out interned))
            {
                return interned;
            }

            interned = new string(NEW_ALLOC_CHAR, value.Length);
            memcpy(interned, value);
            g_intern_table.Add(interned, interned);
#if DBG
            if (log != null)
                log("Interned: " + value);
#endif
        ...
        internal unsafe static gstring internal_format(string input, int num_args)
        {
            ...
            for (int x = 0; x < num_args; x++)
            {
                ...
                fixed (char* ptr_input = input)
                {
                    ...
                    {
                        for (int k = 0; ii < new_len;)
                        {
                            if (j < brace_idx)
                                ptr_result[ii++] = ptr_input[j++];
                            else
                            {
                                ptr_result[ii++] = arg[k++];
                                if (k == arg.Length)
        ...
        internal unsafe static int internal_index_of(string input, char value, int start, int count)
        {
            if (start < 0 || start >= input.Length)
                return -1;//throw new ArgumentOutOfRangeException("start");
        ...
        public static void Initialize(int cache_capacity, int stack_capacity, int block_capacity, int intern_capacity)
        {
            ...
            g_intern_table = new Dictionary<string, string>(intern_capacity);
        ...
        {
            string result = new string(NEW_ALLOC_CHAR, _value.Length);
            memcpy(result, _value);
            return result;
        }
        ...
        public static void Split(gstring input, char flag, ref List<gstring> outList)
        {
            int istart = 0;
            for (int i = 0; i < input.Length; ++i)
            {
                if (input[i] == flag)
                {
                    outList.Add(input.Substring(istart, i - istart));
                    istart = i + 1;
                }
            }
            if (istart < input.Length)
            {
                outList.Add(input.Substring(istart, input.Length - istart));
            }
        }
        #endregion
