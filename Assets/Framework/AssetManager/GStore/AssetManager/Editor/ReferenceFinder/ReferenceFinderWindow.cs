...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    private static ReferenceCache data = new ReferenceCache();
    ...
    //查找资源引用信息
    [MenuItem("Assets/Find References", false, 28)]
    static void FindRef()
    {
        OpenWindow();
    ...
    //打开窗口
    [MenuItem("GStore/Asset/Debug/Reference Finder", false, 1000)]
    ...
    //更新选中资源列表
    public void UpdateSelectedAssets(List<AssetViewItem> guids)
    {
        selectedAssetGuid.Clear();

        foreach (var item in guids)
        {
            selectedAssetGuid.Add(AssetDatabase.AssetPathToGUID(item.data.path));
        }
        needUpdateAssetTree = true;
    }
    ...
    private void UpdateAssetTree()
    {
        if (needUpdateAssetTree && selectedAssetGuid.Count != 0)
        {
            ...
            if (m_AssetTreeView == null)
            {
                ...
                m_AssetTreeView = new AssetTreeView(this, m_TreeViewState, multiColumnHeader);
            }
    ...
    private AssetViewItem SelectedAssetGuidToRootItem(List<string> selectedAssetGuid)
    {
        ...
        {
            root.AddChild(CreateTree(string.Empty, childGuid, ref elementCount, depth));
        }
        updatedAssetSet.Clear();
        return root;
    }
    //通过每个节点的数据生成子节点
    private AssetViewItem CreateTree(string parentGUID, string guid, ref int elementCount, int _depth)
    {
        ...
        {
            id = elementCount,
            displayName = Path.GetFileNameWithoutExtension(referenceData.path),
            data = referenceData,
            depth = _depth
        };
        if (IsInResources(referenceData.path) || IsInBuildSettings(referenceData.path))
        {
            referenceData.state = ReferenceCache.AssetState.BUILTIN;
        }
        ...
        foreach (var childGuid in childGuids)
        {
            ...
            {
                if (referenceData.state == ReferenceCache.AssetState.NORMAL)
                {
                    referenceData.state = ReferenceCache.AssetState.RECURSIVE;
                }
                //防止递归引用
                continue;
            }
            root.AddChild(CreateTree(guid, childGuid, ref elementCount, _depth + 1));
        }
        return root;
    }
    ...
    private static HashSet<string> s_BuiltInScenes = null;
    internal static bool IsInBuildSettings(string path)
    {
        if (path.EndsWith(".unity") == false)
        {
            return false;
        }
        if (s_BuiltInScenes == null)
        {
            s_BuiltInScenes = new HashSet<string>(System.StringComparer.OrdinalIgnoreCase);
            foreach (var scene in EditorBuildSettingsScene.GetActiveSceneList(EditorBuildSettings.scenes))
            {
                s_BuiltInScenes.Add(scene);
            }
        }
        return s_BuiltInScenes.Contains(path);
    }
}
