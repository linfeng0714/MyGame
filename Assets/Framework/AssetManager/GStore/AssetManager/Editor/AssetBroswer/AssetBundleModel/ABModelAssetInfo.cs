...
namespace AssetBundleBrowser.AssetBundleModel
{
    ...
    internal class AssetInfo
    {
        ...
        private HashSet<string> m_ParentBundles;
        ...
        internal AssetInfo(string inName, string bundleName = "")
        {
            ...
            m_ParentBundles = new HashSet<string>();
            isScene = false;
            isFolder = false;
        }
        ...
        internal HashSet<string> parentBundles
        { get { return m_ParentBundles; } }
        ...
        //        else
        //        {
        //            /*string[] allDependencies = AssetDatabase.GetDependencies(m_AssetName, true);
        //            UnityEngine.Debug.Log("---dependencices count---" + allDependencies.Length);*/
        ...
        internal List<AssetInfo> GetDependencies()
        {
            ...
            if (m_dependencies == null)
            {
                Dictionary<string, AssetInfo> dict = new Dictionary<string, AssetInfo>();
                GetDependecies(dict);
                m_dependencies = new List<AssetInfo>(dict.Values);
            }
            return m_dependencies;
        }

        internal void GetDependecies(Dictionary<string, AssetInfo> result, string parentBundleName = "")
        {
            if (result.ContainsKey(m_AssetName))
            {
                //防止循环依赖
                return;
            }
            result[m_AssetName] = this;

            string bundleName = Model.GetBundleName(m_AssetName);
            ...
            {
                m_ParentBundles.Add(bundleName);
            }
            ...
            else
            {
                /*string[] allDependencies = AssetDatabase.GetDependencies(m_AssetName, true);
                UnityEngine.Debug.Log("---dependencices count---" + allDependencies.Length);*/
                foreach (var dep in ReferenceCache.GetDependencies(m_AssetName, false))
                {
                    if (dep != m_AssetName)
                    {
                        var asset = Model.CreateAsset(dep, this);
                        if (asset != null)
                            asset.GetDependecies(result, bundleName);
                    }
                }
            }
        }
