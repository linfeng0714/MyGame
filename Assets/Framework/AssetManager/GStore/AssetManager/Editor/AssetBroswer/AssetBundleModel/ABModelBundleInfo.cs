...
namespace AssetBundleBrowser.AssetBundleModel
{
    internal sealed class BundleTreeItem : TreeViewItem
    {
        ...
        public override string displayName
        {
            get
            {
                ...
                //return AssetBundleBuildEditor.instance.m_ManageTab.hasSearch ? m_Bundle.m_Name.shortName : m_Bundle.displayName;
                return m_Bundle.m_Name.shortName;
            }
        }
    }
    ...
    internal class BundleNameData
    {
        ...
        internal string m_BundleNameWithoutExtension;
        internal string bundleNameWithoutExtension
        {
            get { return m_BundleNameWithoutExtension; }
        }
        ...
        private void SetShortName(string inputName)
        {
            ...
            string[] names = m_ShortName.Split('.');

            m_BundleNameWithoutExtension = names[0];
            //有三段命名的才有变体
            if (names.Length == 3)
            {
                m_VariantName = names[2];
                m_ShortName = string.Format("{0}.{1}", names[0], names[1]);
            }
            else
            {
    ...
    internal abstract class BundleInfo
    {
        ...
        ///// Good Module
        ///// </summary>
        //internal bool isDontDestroy { get; set; }
        internal bool isAlwaysPacked
        {
            get
            {
                return AlwaysPackedSettings.Instance.Contains(displayName);
            }
            set
            {
                AlwaysPackedSettings.Instance.Set(displayName, value);
            }
        }
        internal long originSize
        {
            get
            {
                BundleDataInfo dataInfo = BundleDataInfo.FindBundleDataInfo(this);
                if (dataInfo != null)
                {
                    return dataInfo.GetTotalSize();
                }
                return 0;
            }
        }
        internal long finalSize
        {
            get
            {
                string packedFolder = GStore.AssetPathDefine.editorBundlePath;
                string fullPath = packedFolder + "/" + m_Name.fullNativeName;
        ...
        internal virtual string nameWithoutExtension
        {
            get { return m_Name.bundleNameWithoutExtension; }
        }
    ...
    internal class BundleDataInfo : BundleInfo
    {
        ...
        internal override void RefreshAssetList()
        {
            ...
            //UnityEngine.Debug.Log("--name---" + m_Name.fullNativeName + "--length--" + assets.Length);
            foreach (var assetName in assets)
            {
                ...
                //if (bundleName != m_Name.fullNativeName)
                //{
                //    Debug.LogErrorFormat("bundlleName={0}, fullNativeName={1}", bundleName, m_Name.fullNativeName);
                //}
        ...
        {
            //Debug.Log("----search string---" + search);
            foreach (var asset in m_DependentAssets)
            {
                if (asset.bundleName.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0)
                    return true;
            }
            foreach (var asset in m_ConcreteAssets)
            {
        ...
        /// <returns></returns>
        internal static BundleDataInfo FindBundleDataInfo(AssetBundleModel.BundleInfo bundleInfo)
        {
            BundleDataInfo bundleData = bundleInfo as BundleDataInfo;
            if (bundleData != null)
            {
                return bundleData;
            }
            else
            {
                ...
                BundleFolderInfo bundleFolder = bundleInfo as BundleFolderInfo;
                foreach (var child in bundleFolder.GetChildList())
                {
                    bundleData = FindBundleDataInfo(child);
                    if (bundleData != null)
                    {
                        return bundleData;
                    }
    ...
    internal class BundleFolderConcreteInfo : BundleFolderInfo
    {
        ...
        internal override BundleTreeItem CreateTreeView(int depth)
        {
            ...
            {
                i++;
                EditorTools.DisplayProgressBar(string.Format("AssetBundle载入中...({0}/{1})", i, m_Children.Count), child.Key, (float)i / m_Children.Count);
                var item = child.Value.CreateTreeView(depth + 1);
